...
Use this skill for Chinese or bilingual academic论文 `.docx` files that need GB/T 7714 顺序编码 citation insertion, citation-order repair, or citation semantic alignment review. The default rule is: number citations by first appearance in正文, show正文 citation markers as superscript numeric markers, compress consecutive numbers such as `[1-3]`, use commas only for non-consecutive numbers such as `[1-3,5]`, reuse the same number for repeated citations, and make文末参考文献 match that final order.
...
- Format正文 citation markers as superscript in generated `.docx` files. Compress consecutive numbers with `-`, such as superscript `[1-3]`; use commas only for non-consecutive numbers, such as `[1-3,5]`; do not output separate `[1][2]` markers.
